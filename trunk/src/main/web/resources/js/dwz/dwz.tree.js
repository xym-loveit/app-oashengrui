		...
		genTree:function(options) {
			...
			return this.each(function(){
				...
				if (tree.size()>0) {
					...
					var node_class = node.attr("treeicon") ? node.attr("treeicon") : "file";
					$(">div", node).prepend("<div class='node'></div>"+(op.ckbox?"<div class='ckbox "+checked+"'></div>":"")+(op.icon?"<div class='" + node_class + "'></div>":""));
					addSpace(op.level, node);
