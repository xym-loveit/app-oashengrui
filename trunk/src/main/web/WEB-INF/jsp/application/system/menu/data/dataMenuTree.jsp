				...
				<logic:notEmpty name="root" property="menuChildren">
					<ul>
						<logic:iterate name="root" property="menuChildren" id="child">
							<li treeicon="${child.menuIcon}"><a id="mtlink_${child.id}" <logic:notPresent name='lookup'>class='tparam' param="_var_menuid" paramRel="${child.id}" href="app/system/menu/func.do?action=actionLoadMenuFuncList&menuId=${child.id}" callback="menu_activated(${child.id});" target="ajax" rel="ajBoxMenuFunc"</logic:notPresent> <logic:present name='lookup'>href="javascript:void(0);" onclick="$.bringBack({menuKey:'${child.menuKey}', menuTitle:'${child.menuTitle}'})"</logic:present>><span>${child.menuTitle}</span></a>
