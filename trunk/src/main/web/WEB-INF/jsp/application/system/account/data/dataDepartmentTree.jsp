					   ...
					   <ul>
						<c:if test="${districts ne null}">
							<c:forEach items="${districts}" var="district">
								<li><a id="mtask_disentity_${district.id}" depId="${entity.id}" districtId="${district.id}" href="app/base.do?action=actionLoadEmployeeByDepAndDistrict&districtId=${district.id}&depId=${entity.id}" target="ajax" ref="addressbook_participants"><c:out value="${district.districtName}" /></a></li>
							</c:forEach>
						</c:if>
					   </ul>
					</li>
