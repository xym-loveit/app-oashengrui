...
CREATE TABLE IF NOT EXISTS `app_school_department` (
  ...
  PRIMARY KEY  (`dep_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='学校部门设置' AUTO_INCREMENT=8 ;
...
CREATE TABLE IF NOT EXISTS `app_school_department_position` (
  ...
  PRIMARY KEY  (`pos_id`),
  KEY `dep_id` (`dep_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='部门岗位' AUTO_INCREMENT=2 ;
...
CREATE TABLE IF NOT EXISTS `app_system_log` (
  ...
  PRIMARY KEY  (`log_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='系统日志' AUTO_INCREMENT=13 ;
