...
<div style="line-height: 25px;">
	...
	<div><b>详情点击：</b><a href="javascript:void(0);" 
		onclick="parent.$.pdialog.open(
			'app/personal/task.do?action=dialogAuditPage&taskId=${entity.task.id}&type=${entity.taskApplyType}', 
			'task_applyview_${entity.task.id}',  
			'`${entity.task.taskName}`任务详情',
			{width: 555, height: 445})">点击进入到任务<#if entity.taskApplyType = 1>完成<#else>延期</#if>审批审批窗口</a></div>
</div>