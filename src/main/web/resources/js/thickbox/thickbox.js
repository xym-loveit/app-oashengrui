...
function tb_removeloader(){
	jQuery("#TB_load").remove();
	jQuery("#TB_overlay").remove();
}

// added by jeccyzhao
function tb_showcont(caption,inlinecontent,url){
	caption = caption || "详细信息";
	url = url || "";
	inlinecontent = inlinecontent || "没有任何内容";

	var queryString = url.replace(/^[^\?]+\??/,'');
	var params = tb_parseQuery( queryString );

	TB_WIDTH = (params['width']*1) + 30 || 630; //defaults to 630 if no paramaters were added to URL
	TB_HEIGHT = (params['height']*1) + 40 || 575; //defaults to 570 if no paramaters were added to URL
	...
	jQuery("#TB_ajaxContent").append(inlinecontent);
