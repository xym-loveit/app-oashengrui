		...
		<div class="tabs" currentindex="${currentindex ne null ? currentindex : 0}" eventtype="click">
			<div class="tabsHeader">
				<div class="tabsHeaderContent">
					<ul>
						<li class="selected"><a href="app/finan/${PAGE_TYPE eq 'FE' ? 'expense' : (PAGE_TYPE eq 'FC' ? 'contract' : 'project')}.do?action=loadFina${PAGE_TYPE eq 'FE' ? 'Expense' : (PAGE_TYPE eq 'FC' ? 'Contract' : 'Project')}Records" class="j-ajax" callback="after_switch(1)" paramRef="pagerForm, record_form"><span>审批中</span></a></li>
						<li><a href="app/finan/${PAGE_TYPE eq 'FE' ? 'expense' : (PAGE_TYPE eq 'FC' ? 'contract' : 'project')}.do?action=loadFina${PAGE_TYPE eq 'FE' ? 'Expense' : (PAGE_TYPE eq 'FC' ? 'Contract' : 'Project')}Records&finished" class="j-ajax" callback="after_switch(0)" paramRef="pagerForm, record_form"><span>审批结束</span></a></li>
					</ul>
				</div>
			</div>
